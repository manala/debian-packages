...
ifeq ($(lastword ${DEBIAN_DISTRIBUTION}), wheezy)
	echo "deb http://httpredir.debian.org/debian ${PACKAGE_DISTRIBUTION} main contrib non-free\ndeb-src http://httpredir.debian.org/debian ${PACKAGE_DISTRIBUTION} main contrib non-free" | sudo tee /etc/apt/sources.list.d/${PACKAGE_DISTRIBUTION}.list
	sudo apt-get update
	sudo apt-get install -y --no-install-recommends python-urllib3/testing python-requests/testing tar/testing dpkg-dev/testing linux-libc-dev/testing
else
